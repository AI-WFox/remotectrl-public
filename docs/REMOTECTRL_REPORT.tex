...
\paragraph{Applications.} `app.list` enumerate visible windows. `app.start` dùng preset allowlist, hỗ trợ focus existing hoặc mở new instance. `app.stop` dùng PID và guarded process termination.
...
Command nhạy cảm phải hiện approval prompt trên Agent. `Allow once` chỉ cấp quyền cho command hiện tại. `Allow for this session` chỉ áp dụng cho cùng command type trong phiên hiện tại. `Deny` hoặc đóng cửa sổ bằng X làm command bị từ chối. Session approvals reset khi Agent disconnect, restart hoặc người dùng chủ động reset.
